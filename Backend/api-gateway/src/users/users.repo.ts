...
export class UsersRepo {
  ...
  async getUserForBalanceRequests(db: Pool | PoolClient, userId: string) {
  ...
  // IMPORTANT: uses your real table name: balance_change_requests
  async insertBalanceAddRequest(db: Pool, userId: string, delta: number): Promise<string> {
    const res = await db.query(
      `INSERT INTO app.balance_change_requests (user_id, kind, delta, is_used, created_at)
  ...
  async insertBalanceLog(
    client: PoolClient,
    p: { userId: string; delta: number; balanceAfter: number; reason: string; refId: string; actor: string },
  ) {
    await client.query(
      `INSERT INTO app.balance_logs (user_id, delta, balance_after, reason, ref_type, ref_id, actor, created_at)
  ...
  async sumOpenOrdersTotal(db: Pool | PoolClient, userId: string): Promise<number> {
    const res = await (db as any).query(
  ...
  async insertDirectBalanceLog(client: PoolClient, p: { userId: string; delta: number; balanceAfter: number }) {
    await client.query(
      `INSERT INTO app.balance_logs (user_id, delta, balance_after, reason, actor, created_at)
  ...
  async listUsersAdmin(db: Pool) {
    return db.query(
      `SELECT id, name, email, "class", order_count, balance, blocked, role
       FROM app.users
       ORDER BY name ASC`,
    );
  }
  ...
  async updateAdminDynamic(db: Pool, setClause: string, values: any[]) {
    await db.query(`UPDATE app.users SET ${setClause} WHERE id = $${values.length}`, values);
  }

  async insertUser(db: Pool, p: { name: string; email: string; cls: string | null; role: string }) {
    const res = await db.query(
      `INSERT INTO app.users (name, email, "class", role, blocked, blocked_at, balance, order_count, class_updated_at)
       VALUES ($1, $2, $3, $4, FALSE, NULL, 0, 0, NOW())
       RETURNING id`,
  ...
  async listPendingDeletions(db: Pool) {
    return db.query(
      `SELECT id, user_id, user_name, user_email, user_class, blocked_since, planned_deletion_at
  ...
  async deleteUserAllData(client: PoolClient, userId: string) {
    // exakt nach deinem Schema (Screenshot)
    ...
    await client.query(
      `DELETE FROM app.order_items
       WHERE order_id IN (SELECT id FROM app.orders WHERE user_id = $1)`,
      [userId],
    );

    await client.query(`DELETE FROM app.orders WHERE user_id = $1`, [userId]);

    // pending_registrations (falls vorhanden)
    await client.query(
      `DELETE FROM app.pending_registrations
       WHERE LOWER(email) = (SELECT LOWER(email) FROM app.users WHERE id = $1)`,
      [userId],
    );

    await client.query(`UPDATE app.pending_deletions SET status='CONFIRMED' WHERE user_id=$1`, [userId]);
